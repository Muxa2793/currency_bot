...
def add_notifications_settings(update, context):
    ...
    if asset not in CURRENCY_LIST and asset not in STOCKS_LIST and asset not in CRYPTO_LIST:
        ...
        asset_db_value = asset_db['value']
        sign = '$'
    if value > asset_db_value:
        condition = 'bigger'
        set_notice(asset, value, update, sign, condition)
    elif value < asset_db_value:
        condition = 'smaller'
        set_notice(asset, value, update, sign, condition)


def set_notice(asset, value, update, sign, condition):
    try:
        create_notifications_settings(db, update.effective_user, condition, asset, value)
